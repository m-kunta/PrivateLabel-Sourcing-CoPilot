...
For Pinecone metadata compatibility, `affected_routes` is stored as a comma-separated string; `affected_routes_json` preserves the original route list as JSON.
...
│  (50 synthetic rows)  │         │  └── Mock Events (6)   │
└───────────────────────┘         └────────────────────────┘
```
...
| `disruption_model.py` | Shared disruption classification, coefficients, exposure matching, and risk-row creation |
...
| `requirements-dev.txt` | Runtime dependencies plus pytest for local validation |
